...
first, install [node](https://nodejs.org/en/) & [elm](https://guide.elm-lang.org/install.html).
...
the [Browser.element](https://package.elm-lang.org/packages/elm/browser/latest/Browser#element) function is _critical_ to our app. it takes a record and wires up _other functions_ that tell our app:
...
looking at the first line, we see that `view` is a _function_ that takes as input a `Model` instance and produces a `Html Msg`.
...
at this point, in our app we can see that our model has been successfully updated via the elm debugger:
...
5. grazed over the elm architecture, which we drew direct parallels to react/redux
6. updated our applications state model and messages
7. bound our view layer to a click handler that generated messages that could be reduces
8. fired off a managed side-effect for scrolling the browser, and got a first taste of elm `Task`s, which are akin to lazy promises.

questions? edits? send them my way [here](https://github.com/cdaringe/blog/issues).
